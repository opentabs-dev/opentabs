...
You are an autonomous coding agent working on the OpenTabs Next project.
...
6. Run quality checks: `bun run build && bun run type-check`
...
This is the `__next__` directory inside the OpenTabs monorepo — a migration/rewrite of the Chrome extension + MCP server architecture. It uses:

- **Language**: TypeScript (strict, ES Modules)
- **Runtime**: Bun
- **Build**: `bun run build` (tsc --build), `bun run type-check` (tsc --noEmit)
- **Structure**: `platform/*` (core, mcp-server, browser-extension, plugin-sdk, plugin-loader, etc.) and `plugins/*` (slack, etc.)

All work should be done from the `__next__/` directory as the working root.
...
- ALL commits must pass: `bun run build && bun run type-check`
- Do NOT commit broken code
- Keep changes focused and minimal
- Follow existing code patterns in the `__next__/` codebase
...
- All file paths are relative to the `__next__/` directory