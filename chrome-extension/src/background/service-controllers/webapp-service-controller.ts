 ...
 *
 * Each service is defined by a WebappServiceConfig (pure data + optional
 * callbacks) in service-configs.ts. This single concrete class replaces the
 * previous one-subclass-per-service pattern.
 */

import { dispatchToAdapter } from '../adapter-manager';
import { MessageTypes, createJsonRpcError, JsonRpcErrorCode, isJsonRpcError } from '@extension/shared';
...
class WebappServiceController implements ServiceManager {
  ...
  getConnectionStatus(): ServiceConnection {
    return this.connectionStatus.services[this.config.serviceId];
  }
  ...
  private setConnectionStatus(status: Partial<ServiceConnection>): void {
    Object.assign(this.connectionStatus.services[this.config.serviceId], status);
  }
