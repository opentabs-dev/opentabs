...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      <Table.Cell className="font-medium">`--dangerously-skip-permissions`</Table.Cell>
      <Table.Cell>—</Table.Cell>
      <Table.Cell>Bypass all browser tool confirmation dialogs. **WARNING:** sensitive operations (cookies, storage, script execution) will execute without human approval. For CI and testing environments only.</Table.Cell>
    </Table.Row>
  </Table.Body>
</Table>
