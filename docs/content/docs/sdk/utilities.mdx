...
import {
  ...
  fetchText, buildQueryString, httpStatusToToolError, parseRetryAfterMs,
...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      <Table.Cell>—</Table.Cell>
      <Table.Cell>Build a URL query string from a record, filtering out `undefined` values.</Table.Cell>
    </Table.Row>
    <Table.Row>
...
Builds a URL query string from a record of key-value pairs. Filters out `undefined` values, converts numbers and booleans to strings, and supports arrays (appending multiple values for the same key). Returns the query string without a leading `?`.
