...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      <Table.Cell>When the adapter is being removed — on plugin update or plugin uninstall.</Table.Cell>
    </Table.Row>
    <Table.Row>
...
Called when the adapter is being removed — on plugin update or plugin uninstall. Does not fire when the tab closes or navigates away (the JavaScript context is destroyed by the browser before cleanup code can run). Use it to clean up resources set up in `onActivate()`.
...
When both `onDeactivate` and `teardown` are defined, `onDeactivate` fires first, then `teardown`. `teardown()` fires on plugin update (re-injection) and plugin uninstall. Neither `teardown` nor `onDeactivate` fires when the tab closes or navigates away — the browser destroys the JavaScript context before any cleanup code can run.
